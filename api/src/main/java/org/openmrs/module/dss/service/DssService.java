...
public interface DssService {
    ...
    /**
     * This method tries to load a class based on the rule name and a set of
     * package names First, it tries to load the rule from the rulePackagePrefix
     * package. By default, this package is where the rules that are compiled on
     * the fly live. Second, it tries to load the rule from the
     * defaultPackagePrefix package. By default, this package contains the
     * default library of pre-compiled rules Finally, it tries to load the rule
